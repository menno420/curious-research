# curious-research — agent orientation & reading order
...
- **Architecture / layering:** flat lanes, no layering rules: guides/ (visual explainers), ideas/ (one file per idea), projects/ (finished builds with their docs), plus workshop lanes (printing/, arduino/, arm/) added as needed - keep each thing in its lane
- **Ownership** (who owns each write path): one human owner (the friend) plus his Claude; every change lands via a PR he can read; nothing merges that he could not understand from the PR description
- **Mutation seam** (how writes are gated): no runtime or database here - the seam is the PR itself: branch, PR, green substrate-gate, merge; nothing lands on main directly
